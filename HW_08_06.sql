...
SELECT * FROM Customers 
				 WHERE Phone = '(053)3456788';               
-- выборка даных низкая

CREATE INDEX ixPhone
ON Customers(Phone);

SELECT * FROM Customers 
				 WHERE CustomerNo = 4; -- выборка даных низкая

SELECT * FROM Customers 
				 WHERE Phone = '(053)3456788';
--  после добавления индекса в столбец Phone выборка данных улучшилась
...
--  таблица Customers с первичным ключом
CREATE TABLE Customers
	(
	CustomerNo int NOT NULL auto_increment,
	FName nvarchar(15) NOT NULL,
...
SELECT * FROM Customers 
				 WHERE CustomerNo = 4; -- с первичным ключом выполнение запроса высокое

SELECT * FROM Customers 
				 WHERE City = 'Чернигов'; -- без первичного ключа и индекса выполнение запроса низкое

CREATE INDEX ixLName
ON Customers(LName);

SELECT * FROM Customers 
				 WHERE LName = 'Выжлецов'; -- после добавления индекса выборка данных стала быстрее

-- на скорость выполнения выборки данных в таблице влияет наличие первичного ключа и наличие индекса 
